...
export default {
  data() {
    ...
    var validatefrozenBoxCode = (rule, value, callback) => {
      ...
      } else if (!/^CS-T1[0-9]{1}F[0-9]{3}$/.test(value)) {
    ...
    return {
      ...
      downloadUrl:
        SERVER_NAME + "/base/hospital/sample/stoolSampleQueryExcel?type=stool",
      btnAuth: {
  ...
  mounted() {
    ...
    this.query(
      this.$store.state.fecalListPageNo,
      this.$store.state.fecalListPageSize
    );
  },
  beforeDestroy() {
  ...
  methods: {
    ...
    getSidInfo() {
      ...
      this.addForms.frozenBoxCode = "CS-T1" + num + "F";
    },
    getInfo(value) {
