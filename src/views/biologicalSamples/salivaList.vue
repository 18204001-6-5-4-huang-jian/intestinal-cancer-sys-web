    ...
    <div class="content">
      <div class="filter-container">
        ...
        <el-form :model="qc" :inline=true class="clear">
          ...
          <div>
            <el-button size="small" type="primary" icon="el-icon-search" @click="query(1,$store.state.salivaListPageSize)">查询</el-button>
            <el-button  type="primary" size="small" icon="el-icon-close" @click="reset" >重置</el-button>
          </div>
        </el-form>
      </div>
      <el-dialog :visible.sync="addFormDialog" title="唾液样本" :show-close="false">
        ...
        <el-table
          ...
          >
            <template slot-scope="scope">
              ...
              <el-button type="text" @click="applyEdit(scope.row)" v-if="scope.row.collectStatus == 1 && scope.row.applyStatus==0 && !(scope.row.courierStatus==1 || scope.row.courierStatus==3)">申请编辑</el-button>
              <el-button type="text" @click="del(scope.row)" 
              v-if="scope.row.collectStatus == 1 && scope.row.applyStatus==0 && !(scope.row.courierStatus==1 || scope.row.courierStatus==3)">删除</el-button>
...
export default {
  data() {
    ...
    return {
      ...
      downloadUrl:
        SERVER_NAME + "/base/hospital/sample/stoolSampleQueryExcel?type=saliva",
      btnAuth: {
  ...
  mounted() {
    ...
    this.query(
      this.$store.state.salivaListPageNo,
      this.$store.state.salivaListPageSize
    );
  },
  beforeDestroy() {
    this.$store.state.salivaListPageNo = 1;
    this.$store.state.salivaListPageSize = 10;
  },
  methods: {
    // 申请编辑
    applyEdit(row) {
    ...
    updateCourierResult(row) {
      ...
      $axios_http({
        ...
        this.insertForm.collectStatusDateBySql = new Date(
          res.data.data[0].collectStatusDate
        );
        ...
        if (valid) {
          if (this.editBtn == true) {
            ...
            $axios_http({
              ...
              this.editBtn = false;
              this.$refs[formName].resetFields();
              this.query(1, this.$store.state.salivaListPageSize);
            });
          } else {
            // 新增
            $axios_http({
    ...
    query(pageNo, pageSize) {
      $axios_http({
        ...
        this.$store.commit("get_salivaListPageNo", pageNo);
    ...
    pageSizeChange(pageSize) {
      this.$store.commit("get_salivaListPageSize", pageSize);
      //this.queryResult.pageSize = pageSize
      this.query(this.$store.state.salivaListPageNo, pageSize);
    },
    //切换当前页事件，做重新查询操作
    currentPageChange(currentPage) {
      this.$store.commit("get_salivaListPageNo", currentPage);
      //this.queryResult.pageNo = currentPage
      this.query(currentPage, this.$store.state.salivaListPageSize);
    }
  }
};
</script>
