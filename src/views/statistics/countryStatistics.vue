<template>
  <div slot="right" class="content-page" v-if="countryStatistics_page">
    <div class="content">
      <div class="filter-container" style>
        ...
        <el-form :model="qc" :inline="true">
          ...
          <el-date-picker
            ...
            <el-button size="small" type="primary" icon="el-icon-search"  @click="queryClick" v-if="btnAuth.country_statistics_query_btn">查询</el-button>
            <el-button type="primary" size="small" icon="el-icon-close" @click="reset" v-if="btnAuth.country_statistics_query_btn">重置</el-button>
          <!-- <div style="margin-top:10px">
            <el-button type="primary" size="small"  @click="jump" v-if="btnAuth.country_statistics_query_btn">智能数据统计平台<i class="el-icon-arrow-right el-icon--right"></i></el-button>
          </div> -->
        </el-form>
        ...
        <!--</div>-->
        <!-- <div v-for="(item,index) in tableData" :key="index" class="community-item">
        ...
        <!-- <div class="bj">
          ...
          <el-table
            :data="countryTableDataPathological"
            ...
            <el-table-column
              ...
              <el-tabs style="margin:20px 0;" v-model="activeName" type="card" @tab-click="handleClick">
            <el-tab-pane label="随访任务量-分地区和分月份" name="1">
                 <commonTable :tableData="taskNumtableData"></commonTable>
            </el-tab-pane>
            <el-tab-pane label="随访任务进度表格-分地区和组别" name="2">
               <taskprocessTable :tableData="taskprocesstableData"></taskprocessTable>
            </el-tab-pane>
            <el-tab-pane label="FIT检查完成情况-分地区和组别" name="3">
              <fitTable :tableData="fittableData"></fitTable>
            </el-tab-pane>
            <el-tab-pane label="结肠镜完成情况-分地区和组别" name="4">
              <taskcolorectalTable :tableData="taskcolorectaltableData"></taskcolorectalTable>
            </el-tab-pane>
             <el-tab-pane label="病变检出情况" name="5">
               <pathologyTable :tableData="pathologytableData"></pathologyTable>
            </el-tab-pane>
          </el-tabs>
      </div>
    </div>
...
<script>
import commonTable from './tasknumtableArea.vue'
import taskprocessTable from './taskprocesstableArea.vue'
import fitTable from './fittableArea.vue'
import taskcolorectalTable from './taskcolorectaltableArea.vue'
import pathologyTable from './pathologytableArea.vue'
  export default {
    name: 'Right',
    components:{
      commonTable,
    ...
    data () {
      return {
        ...
        //权限判定
        countryStatistics_page:false,
        btnAuth:{
          country_statistics_query_btn:false,
        },
        //查询条件
        "qc":{
          "startDate":null,
    ...
    mounted(){
      ...
      let obj = this.checkPageAuth(this,"countryStatistics_page",this.btnAuth);
      this.query();
    },
    methods:{
      //跳转到bi
...
//            window.open('http://192.168.1.16/login?userId='+window.localStorage.getItem('loginName'), '_blank');
            window.open('http://111.202.40.136:9300/login?userId='+window.localStorage.getItem('loginName'), '_blank');
          }
      ...
      query(){
//         $axios_http({
//           url:"/base/hospital/person/listGroups/groupSumsByNation",
//           data:{
...
//             obj.DNANoFeedbackNum = this.tableData[i].list[2].DNANoFeedbackNum +this.tableData[i].list[3].DNANoFeedbackNum +this.tableData[i].list[4].DNANoFeedbackNum
//             obj.DNANoGenerate = this.tableData[i].list[2].DNANoGenerate +this.tableData[i].list[3].DNANoGenerate +this.tableData[i].list[4].DNANoGenerate
...
//             this.tableData[i].list[6].name = this.tableData[i].areaName
//             this.countryTableData.push(this.tableData[i].list[6])
//           }
//           // 地区的合计
//             let $groupsNum=0;
...
//             let $FitResultNum=0;
//             let $SearchOutNum=0;
//             for(let i=0;i<this.countryTableData.length;i++){
//               $groupsNum += this.countryTableData[i].groupsNum
//               $bookingColonscopyNum += this.countryTableData[i].bookingColonscopyNum
//               $inspectColonscopyNum += this.countryTableData[i].inspectColonscopyNum
//               $DNANum += this.countryTableData[i].DNANum
//               $DNAFeedbackNum += this.countryTableData[i].DNAFeedbackNum
//               $DNANoFeedbackNum += this.countryTableData[i].DNANoFeedbackNum
//               $DNANoGenerate += this.countryTableData[i].DNANoGenerate
//               $FitNum += this.countryTableData[i].FitNum
//               $FitResultReturnNum += this.countryTableData[i].FitResultReturnNum
//               $FitPositiveNum += this.countryTableData[i].FitPositiveNum
//               $FitNegativeNum += this.countryTableData[i].FitNegativeNum
//               $FitInvalidNum += this.countryTableData[i].FitInvalidNum
//               $FitResultNum += this.countryTableData[i].FitResultNum
//               $SearchOutNum += this.countryTableData[i].SearchOutNum
...
//               DNANoFeedbackNum:$DNANoFeedbackNum,
//               DNANoGenerate:$DNANoGenerate,
//               FitNum:$FitNum,
...
//             }
//             this.countryTableData.push(sumobj)
//         })
          // 随访任务量
        $axios_http({
          url:"/base/hospital/statistics/nation/followTask",
          data:{
            startDate:this.qc.startDate,
            endDate:this.qc.endDate,
          },
          vueObj:this
           ...
           if(res.data.statusMsg == 'success'){
             this.taskNumtableData = res.data.data;
           }
        })
        // 随访任务进度 
         $axios_http({
          url:"/base/hospital/statistics/nation/followSchedule",
          data:{
            startDate:this.qc.startDate,
            endDate:this.qc.endDate,
          },
          vueObj:this
           ...
           if(res.data.statusMsg == 'success'){
             this.taskprocesstableData = res.data.data;
           }
        })
        // FIT检查完成情况
         $axios_http({
          url:"/base/hospital/statistics/nation/fitSchedule",
          data:{
            startDate:this.qc.startDate,
            endDate:this.qc.endDate,
          },
          vueObj:this
           ...
           if(res.data.statusMsg == 'success'){
             this.fittableData = res.data.data;
           }
        })
        // 结直肠镜完成情况
          $axios_http({
          url:"/base/hospital/statistics/nation/cjSchedule",
          data:{
            startDate:this.qc.startDate,
            endDate:this.qc.endDate,
          },
          vueObj:this
           ...
           if(res.data.statusMsg == 'success'){
             this.taskcolorectaltableData = res.data.data;
           }
        })
        // 病变检出
        $axios_http({
          url:"/base/hospital/person/listGroups/lesionStatisticsByNation",
          data:{
            startDate:this.qc.startDate,
            endDate:this.qc.endDate,
          },
          vueObj:this
           ...
           if(res.data.statusMsg == 'success'){
              ...
              item.list[5] = item.list[6]
              item.list.pop();
              })
              this.pathologytableData = res.data.data;
           }
        })
      },
      //重置检索条件
